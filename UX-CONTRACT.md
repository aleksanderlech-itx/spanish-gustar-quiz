...
- Topic detail, the round and flashcards each show a "?" help button at the end of their header
  ...
  link to the matching `/how-to-use` section. That link opens in a new tab, with a small note saying
  so, so an in-progress round or flashcard session is never lost. The dialog closes on Escape, the ✕ button or a backdrop click, and
